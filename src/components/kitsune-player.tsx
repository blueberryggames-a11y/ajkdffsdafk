...
function AniBlossomPlayer({
  ...
  useEffect(() => {
    ...
    const fetchBookmarkAndWatchedId = async () => {
      ...
      // Find the specific watched record for THIS episode from Firestore
      try {
        const whSnap = await getDocs(
          collection(db, "users", auth.id, "watchHistory", id, "episodes")
        );

        if (!isMounted) return;

        const currentEpNo = parseInt(serversData.episodeNo);
        const existingWatched = whSnap.docs.find((d) => {
          const w = d.data();
          return (
            (currentEpNo && Number(w.episodeNumber) === currentEpNo) ||
            (w.episodeId && w.episodeId === serversData.episodeId)
          );
        });

        if (existingWatched) {
          const w = existingWatched.data();
          watchedRecordIdRef.current = existingWatched.id;
          initialSeekTimeRef.current = typeof w.current === "number" ? w.current : null;
...
export default AniBlossomPlayer;
